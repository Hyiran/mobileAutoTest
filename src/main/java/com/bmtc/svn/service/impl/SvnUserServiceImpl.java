...
public class SvnUserServiceImpl implements SvnUserService 
{
	...
	@Transactional
	@Override
	public int addSvnUser(SvnUser svnUser) throws Exception {
		...
		svnService.exportConfigToSvnServer(svnUser.getSvnRepoName(), bmtcConfig.getSvnConfFilesLocation());
	...
	@Transactional
	@Override
	public int deleteSvnUser(long svnUserId, String svnUserName, String svnRepoName) throws Exception {
		logger.info("SvnUserServiceImpl.deleteSvnUser() start");

		// 删除SVN用户之前，先根据根据svnUserName和svnRepoName，删除该用户在svnRepoName仓库下的所有权限
		// 根据svnUserId删除用户SVN权限
		int res = 0;
		res = svnUserRightDao.deleteSvnUserRightBySvnUserId(svnUserId);
		if(res == -1) {
			logger.info("svn_user_authz表中不存在svn用户名为'" + svnUserName + "', 库名为'" + svnRepoName + "'的记录");
		} else if(res > 0) {
			logger.info("根据svnUserName和svnRepoName删除" + res + "条SVN用户权限信息完成");
		}
	...
	@Transactional
	@Override
	public int updateSvnUser(SvnUser svnUser) throws Exception {
	...
	@Transactional
	@Override
	public int saveSvnUser(SvnUser svnUser) throws Exception {
